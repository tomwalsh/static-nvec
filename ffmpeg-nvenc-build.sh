...
installAptLibs() {
    ...
    sudo apt-get -y --force-yes install $PKGS \
      build-essential cmake pkg-config texi2html software-properties-common \
...
installYumLibs() {
    sudo yum -y install $PKGS cmake3 freetype-devel gcc gcc-c++ pkgconfig zlib-devel \
...
compileLibX264() {
    ...
    cd "$WORK_DIR/"
    X264="x264-snapshot-20191217-2245.tar.bz2"
    Wget http://download.videolan.org/pub/x264/snapshots/$X264
    rm -rf x264-snapshot*/ || :
    tar xjvf $X264
    cd x264-snapshot*
